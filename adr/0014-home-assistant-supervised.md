...
By not using the Home Assistant Operating System, the user is responsible for making sure that all required components are installed and maintained. Required components and their versions will change over time. Home Assistant is not responsible for issues that come up with your host or config/resource conflicts caused by other applications installed on the host system.
